...
Kindleを前面に表示した状態でキャプチャします。実行中はKindleウィンドウに触れないでください。
...
Win32 API を使い、Kindleが背面にある状態でもキャプチャ・ページ送りが可能です。**実行中も他のアプリで自由に作業できます。**

```bash
# 縦書き本（バックグラウンド）
python kindle_capture.py --background

# 横書き本（バックグラウンド）
python kindle_capture.py --background --horizontal
```

> **Note**: バックグラウンドモードではKindleを前面にする必要はありません。ただし、Kindleウィンドウを**最小化しないでください**（最小化するとキャプチャが空白になります）。
...
# 日本語OCR付き（テキスト検索可能）
python make_pdf_win.py --ocr

# 縦書き本のOCR（精度向上）
python make_pdf_win.py --ocr --lang jpn_vert

# ディレクトリとファイル名を指定
python make_pdf_win.py --input-dir ~/Desktop/KindleScreenshots_Horizontal --output book.pdf

# 横書き本のOCR + 出力先指定
python make_pdf_win.py --ocr --lang jpn --input-dir ~/Desktop/KindleScreenshots_Horizontal --output book_ocr.pdf
```
...
| 機能 | macOS版 | Windows版 |
|------|---------|-----------|
| ウィンドウ操作 | AppleScript | pygetwindow / Win32 API |
| スクリーンショット | screencapture | pyautogui / PrintWindow API |
| キー送信 | osascript (key code) | pyautogui / PostMessage API |
| バックグラウンド実行 | 非対応 | `--background` で対応 |
| ページ送り方向 | 別ファイル (v7 / horizontal) | `--horizontal` フラグで切替 |
| PDF変換 | ImageMagick + ocrmypdf | img2pdf + ocrmypdf |
| 重複ページ検出 | なし | あり（自動停止） |
| OCR | ocrmypdf (日本語) | ocrmypdf (日本語 + 縦書き対応) |

## OCR セットアップ（オプション）

`--ocr` フラグを使うには Tesseract OCR が必要です。
...
インストーラーにデフォルトで `eng` のみ含まれるため、日本語データを手動で追加します。

1. [tessdata_best](https://github.com/tesseract-ocr/tessdata_best) から以下をダウンロード:
   - `jpn.traineddata` — 日本語（横書き）
   - `jpn_vert.traineddata` — 日本語（縦書き）
2. `C:\Program Files\Tesseract-OCR\tessdata\` にコピー（管理者権限が必要）

### 3. 動作確認

```bash
tesseract --list-langs
# jpn, jpn_vert が表示されればOK
```
