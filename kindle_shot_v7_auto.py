...
# 保存先ディレクトリの作成
SAVE_DIR = os.path.expanduser("~/Desktop/KindleScreenshots")
...
def detect_modal(screenshot_path, template_path, threshold=0.8):
    ...
    try:
        ...
        print(f"Current max_val for modal detection: {max_val:.2f} (Threshold: {threshold:.2f})")
...
def main():
    ...
    print("Kindle 縦書き本対応版(v7) - 左矢印でページ送り (自動停止機能付き)")
    ...
    while True:
        ...
        success, current_screenshot_path = capture_kindle_window_safe(index, temp_filepath="/tmp/temp_kindle_screenshot.png")
        if not success:
        ...
        print(f"Sending Left Arrow key (Next Page for Japanese books)...")
        turn_page()
        index += 1
